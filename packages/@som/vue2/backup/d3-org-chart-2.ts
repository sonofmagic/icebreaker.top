...
import type { OrgChart as OutOrgChart } from 'd3-org-chart'
...
declare module 'd3-selection' {
  export interface Selection<
    ...
    PDatum
  > {
    patternify<DescElement extends BaseType>(params: {
      selector: string
      tag: string
      data?: any
...
export class OrgChart<T> implements OutOrgChart<T> {
  constructor () {
    // Exposed variables
    const attrs: Attrs<T> = {
  ...
  getNodeChildren (
    {
      ...
      children,
      _children
    }: HierarchyNode<T> & { _children: HierarchyNode<T>[] },
    nodeStore: T[]
  ) {
