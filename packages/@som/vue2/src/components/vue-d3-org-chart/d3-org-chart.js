...
export class OrgChart {
    ...
    const attrs = {
      ...
      nodeUpdate: function (d, i, arr) {
        d3.select(this)
          .select(".node-rect")
          .attr("stroke", (d) =>
            d.data._highlighted || d.data._upToTheRootHighlighted ? "#152785" : "none"
          )
  ...
  addNode(obj) {
    ...
    if (!parentFound) {
      console.log(
        `ORG CHART - ADD - Parent node with id "${attrs.parentNodeId(obj)}" not found in the tree`
      );
      return this;
    }
  ...
  restyleForeignObjectElements() {
    ...
    attrs.svg
      ...
      .each(function (d, i, arr) {
        // console.log(d.data.id, d.data._mounted);
        if (!d.data._mounted) {
          /**
  ...
  onButtonClick(event, d) {
    ...
    if (d.children) {
      ...
      //   c.data._expanded = false;
      //   // this.setMountedFlagToChildren(c, false);
      //   c.data._mounted = true;
      // });
      this.setExpansionFlagToChildren(d, false);
    } else {
      // Expand children
      ...
      if (d.children) {
        d.children.forEach((c) => {
          ...
          // this.setMountedFlagToChildren(c, false);
          // c.data._mounted = false;
          this.setMountedFlagToChildren(c, false);
          // this.setMountedFlagToChildren(c, false);
        });
        // this.setMountedFlagToChildren(d, false);
      }
    }
  ...
  setLayouts({ expandNodesFirst = true }) {
    ...
    attrs.root = d3.hierarchy(attrs.data);
    //.id((d) => attrs.nodeId(d))
