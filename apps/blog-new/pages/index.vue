<template>
  <NuxtLayout name="index">
    ...
    <swiper
      ...
      <swiper-slide v-for="(key, idx) in dates" :key="key">
        <!-- lg:h-[calc(100vh-108px)] -->
        <div class="relative px-4 py-4 lg:py-8">
          <!-- <ContentList v-slot="{ list }" path="/articles" :query="query"> -->
          <div class="space-y-8">
            <BaseCard
              v-for="article in groupedArticles[key]"
              :key="article._path">
              <div class="relative">
...
import { Pagination, Swiper as SwiperCls } from 'swiper'
...
const pagination = {
  ...
  //   return '<span class="' + className + '">' + dates.value[index] + '</span>'
  // }
}
definePageMeta({
  layout: false
...
function onSlideChange(swiper: SwiperCls) {
